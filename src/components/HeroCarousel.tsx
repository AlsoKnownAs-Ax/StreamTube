...
const HeroCarosel = ( { movies }: Props) => {  
  ...
  return (
    <div className="overflow-hidden cursor-pointer relative" ref={emblaRef}>
        ...
        { movies.map((movie) => (
          <div key={movie?.id} className='flex-full min-w-0 relative'>
            <Image 
              ...
              height={1080}
            />
            <div className='hidden lg:inline absolute top-0 
            pt-40 xl:pt-72 left-0 bg-transparent
            z-20 h-full w-full bg-gradient-to-r from-gray-900/90 
      ...
      bg-gradient-to-b from-gray-900/10 via-gray-900/30
      to-gray-300 dark:to-[#121212]'/>
    </div>
  )
}
